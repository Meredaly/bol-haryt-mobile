  ...
  <ul class="select-pop-up__list">
    <template v-for="item in items" :key="item.id">
      <li class="select-pop-up__item-box">
        <button @click="selectItem(item)" :class="['select-pop-up__item', { _active: selectedIds.includes(item.id) }]">
          <div class="select-pop-up__checkbox">
            <base-icon :name="selectedIds.includes(item.id) ? 'checked' : 'unchecked'" />
          </div>
